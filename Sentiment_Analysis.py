...
srt_list = [file for file in glob.glob("*.srt")]
...
def srt_df_dict_creator(srt_list):
	...
	for filename in srt_list:
		print(filename)
		try:	srt_dict[filename] = dialogue_dataframe_extractor(filename).to_json()
		except:	pass
	return(srt_dict)
...
# with open('all_srt.json', 'w') as fp:
#     json.dump(all_srt_json, fp)


with open('all_srt.json') as json_file:
    data = json.load(json_file)
    i = 0
    for p in data.keys():
    	print(i+1)
    	print(p)    	
    	print ('True' if data.get(p) else 'False')
    	i += 1
